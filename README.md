...
5. 틀린 얼굴을 클릭하면 **정답이 어느 얼굴이었는지 공개**하고 그 라운드를 다시 진행하지만, **오답 1회당 최종 티어가 1단계 하락**합니다.
6. 결과 화면에서 평균 기록·티어·라운드별 기록을 보여주고, 닉네임을 입력해 랭킹에 저장할 수 있습니다.
7. 게임 중에는 화면 상단에 **현재 라운드·남은 기억 시간·현재 평균·예상 티어**가 표시되어, 오답이 나면 예상 티어가 즉시 떨어지는 것을 볼 수 있습니다.
8. 랭킹 TOP 20은 시작 화면과 결과 화면에서 모두 볼 수 있고, 목록을 스크롤해 하위 순위까지 확인할 수 있습니다.
...
**오답 패널티**: 오답 1회당 티어가 1단계 하락하며, 최하위(아이언)에서 더 내려가지는 않습니다.
예를 들어 평균이 에메랄드 구간이어도 오답이 2회면 최종 티어는 골드가 됩니다.

개인 최고 기록은 브라우저(localStorage)에 저장되어 시작 화면에 내 티어로 표시됩니다.
최고 기록 판정은 **패널티가 적용된 최종 티어**가 우선이고, 티어가 같으면 평균이 낮은 판이 이깁니다.
(전역 랭킹에 저장되는 값은 평균 기록 자체이며 패널티는 반영되지 않습니다.)
